...
def set_static_mesh(blueprint_name: str, component_name: str, mesh_type: str) -> bool:
    ...
    if not response or response.get("status") != "success":
        logger.error(
            f"Failed to set static mesh '{mesh_type}' for component '{component_name}': {response}"
        )
        return False

    logger.info(
        f"Static mesh '{mesh_type}' set for component '{component_name}' successfully!"
    )
    return True
...
def spawn_blueprint_actor(
    blueprint_name: str, actor_name: str, location: List[float] = []
) -> bool:
...
def test_static_mesh_components():
    ...
    if not spawn_blueprint_actor(
        bp_sphere_name, "SphereMeshActor", [100.0, 0.0, 100.0]
    ):
        return False
    ...
    if not spawn_blueprint_actor(
        bp_cylinder_name, "CylinderMeshActor", [0.0, 100.0, 100.0]
    ):
        return False
...
def test_collision_components():
    ...
    if not add_component(
        ...
        location=[0.0, 0.0, 0.0],
        scale=[1.0, 1.0, 1.0],
        properties={"bVisible": True, "CollisionProfileName": "BlockAll"},
    ):
        return False
    ...
    if not spawn_blueprint_actor(
        bp_sphere_name, "SphereCollisionActor", [100.0, 0.0, 100.0]
    ):
        return False
...
def test_scene_component_hierarchy():
    ...
    if not set_static_mesh(
        blueprint_name=bp_name, component_name=cube_component_name, mesh_type="Cube"
    ):
        return False
    ...
    if not set_static_mesh(
        blueprint_name=bp_name, component_name=sphere_component_name, mesh_type="Sphere"
    ):
        return False
