...
def main():
    ...
    try:
        ...
        bp_params = {"name": "TestCompRefBP", "parent_class": "Actor"}
        ...
        logger.info(
            f"BeginPlay event node added successfully with ID: {begin_play_node_id}"
        )
        ...
        response = send_command(
            sock, "add_blueprint_get_self_component_reference", get_component_params
        )
        if not response or response.get("status") != "success":
        ...
        logger.info(
            f"Component reference node added successfully with ID: {comp_ref_node_id}"
        )
        ...
        logger.info(
            f"AddForce function node added successfully with ID: {function_node_id}"
        )
        ...
        if (
            ...
            or not response.get("result", {}).get("success", False)
        ):
            ...
            for pin_name in fallback_pin_names:
                ...
                response = send_command(
                    sock, "connect_blueprint_nodes", connect_target_params
                )
                if (
                    response
                    and response.get("status") == "success"
                    and response.get("result", {}).get("success", False)
                ):
        ...
        compile_params = {"blueprint_name": "TestCompRefBP"}
