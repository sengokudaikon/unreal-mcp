...
def main():
    ...
    try:
        ...
        bp_params = {"name": "BirdBP", "parent_class": "Pawn"}
        ...
        response = send_mcp_command(
            ...
            {"action_name": "Flap", "key": "SpaceBar", "input_type": "Action"},
        )
        ...
        if response and response.get("status") == "success":
            ...
            if nodes:
                ...
                begin_play_response = {
                    "result": {"success": True, "node_id": begin_play_node_id}
                }
        ...
        response = send_mcp_command(
            "add_blueprint_input_action_node", input_action_params
        )
        ...
        response = send_mcp_command(
            "add_blueprint_get_self_component_reference", get_component_params
        )
        ...
        if not response or response.get("status") != "success":
            ...
            for target in targets_to_try:
                ...
                response = send_mcp_command(
                    "add_blueprint_function_node", function_params
                )
                if response and response.get("status") == "success":
                    logger.info(
                        f"Successfully added AddImpulse using target class: {target}"
                    )
                    break
            else:
        ...
        response = send_mcp_command("compile_blueprint", {"blueprint_name": "BirdBP"})
        ...
        get_camera_params = {
            ...
            "params": {"ActorClass": "/Script/Engine.CameraActor"},
        ...
        if response and response.get("status") == "success":
            ...
            if nodes:
                ...
                response = send_mcp_command(
                    "add_blueprint_event_node", begin_play_params
                )

                if not response or response.get("status") != "success":
                    logger.error(
                        f"Failed to get/create ReceiveBeginPlay node: {response}"
                    )
                    return
        ...
        response = send_mcp_command(
            "connect_blueprint_nodes", connect_begin_play_params
        )
        ...
        response = send_mcp_command(
            "connect_blueprint_nodes", connect_camera_exec_params
        )

        if not response or response.get("status") != "success":
            logger.error(
                f"Failed to connect GetActorOfClass to SetViewTargetWithBlend execution: {response}"
            )
            return

        logger.info(
            "Connected GetActorOfClass execution to SetViewTargetWithBlend successfully!"
        )
        ...
        if not response or response.get("status") != "success":
            logger.error(
                f"Failed to connect camera to SetViewTargetWithBlend: {response}"
            )
            return
        ...
        get_pc_params = {
            ...
            "node_position": [
                0,
                -100,
            ],  # Place between GetActorOfClass and SetViewTarget
        }
        ...
        if not response or response.get("status") != "success":
            logger.error(
                f"Failed to connect player controller to SetViewTargetWithBlend: {response}"
            )
            return

        logger.info(
            "Connected PlayerController to SetViewTargetWithBlend target successfully!"
        )
        ...
        response = send_mcp_command(
            "create_actor",
            {
                ...
                "location": [
                    ...
                    0.0,
                    250.0,
                ],  # Position camera to view the bird from a distance
        ...
        logger.info(
            "You can now press spacebar to make the bird flap! The camera will automatically view the bird."
        )
