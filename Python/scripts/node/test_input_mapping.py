...
def setup_input_mapping(
    sock: socket.socket, action_name: str, key: str, input_type: str = "Action"
) -> bool:
...
def main():
    ...
    try:
        ...
        bp_params = {"name": "InputControllerBP", "parent_class": "Actor"}
        ...
        if (
            ...
            or not response.get("result", {}).get("success")
        ):
        ...
        function_params = {
            ...
            "params": {"InString": "Input Controller Initialized", "Duration": 5.0},
            "node_position": [250, 0],
        }
        ...
        for action_name in ["Jump", "Pause", "Restart"]:
            ...
            response = send_command(
                sock, "add_blueprint_function_node", function_params
            )
            ...
            if (
                ...
                logger.error(
                    f"Failed to add function node for {action_name}: {response}"
                )
                return
        ...
        for action_name in ["BeginPlay"] + list(action_positions.keys())[
            :3
        ]:  # BeginPlay + first 3 actions
            # Close and reopen connection
            sock.close()
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.connect(("127.0.0.1", 55557))
        ...
        compile_params = {"blueprint_name": "InputControllerBP"}
