...
def _handle_service_result(result: Dict[str, Any], operation: str) -> Dict[str, Any]:
    ...
    # Check if the response indicates an error from Unreal
    if result.get("status") == "error":
        logger.error(f"Unreal error in {operation}: {result.get('error')}")
        return {
            "success": False,
            "message": result.get("error", "Unknown Unreal error"),
        }
